...
// password
func npmrcContentsUsernamePassword(settings Settings) string {
	// get the base64 encoded string
	authString := fmt.Sprintf("%s:%s", settings.Username, settings.Password)
	encoded := base64.StdEncoding.EncodeToString([]byte(authString))

	// create the file contents
	return fmt.Sprintf("_auth = %s\nemail = %s", encoded, settings.Email)
}

/// Writes npmrc contents when using a token
func npmrcContentsToken(settings Settings) string {
	registry, _ := url.Parse(settings.Registry)
	return fmt.Sprintf("//%s/:_authToken=%s", registry.Host, settings.Token)
}
